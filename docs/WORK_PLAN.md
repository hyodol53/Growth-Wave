...
| 6.2 | 프로젝트 관리 UI 개발 (실장 이상) | `[ ]` | Agent-Frontend | `FR-A-6.x`. 프로젝트 목록 조회, 생성, 수정, 멤버 배정 |
| 6.3 | 평가 설정 UI 개발 (관리자/실장) | `[ ]` | Agent-Frontend | `FR-A-1.3, 2.1, 2.2`. 참여 비중, 평가 기간, 등급 비율 설정 |
| 6.4 | 평가 진행 UI 개발 (전체) | `[ ]` | Agent-Frontend | `FR-A-3.x`. PM동료평가, PM평가, 정성평가 제출 화면 |
