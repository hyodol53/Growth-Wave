...
*   **메뉴 및 콘텐츠 한글 작성**: 사용자 인터페이스(UI)의 모든 메뉴 항목, 버튼 텍스트, 페이지 제목, 설명, 오류 메시지 등 사용자에게 노출되는 모든 텍스트 콘텐츠는 **반드시 한국어로 작성해야 합니다.** 이는 한국인 사용자를 위한 서비스 현지화(Localization)의 핵심 요구사항입니다. 기존 영문 텍스트가 있다면 한국어로 번역하여 적용합니다.
